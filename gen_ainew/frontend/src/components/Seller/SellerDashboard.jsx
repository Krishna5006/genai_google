...
const SellerDashboard = ({ user, setUser, onLogout }) => {
  ...
  useEffect(() => {
    const fetchSellerData = async () => {
      try {
        ...
        // Fetch seller product count (auth-based, then phone-based, then global fallback)
        try {
          const token = getToken();
          const headers = token ? { headers: { Authorization: `Bearer ${token}` } } : {};
          let total = 0;
          try {
            const myRes = await api.get('/products/my-products?limit=1', headers);
            total = myRes?.data?.pagination?.totalProducts || 0;
          } catch (myErr) {
            console.warn('Could not fetch my products count (likely unauthenticated):', myErr?.response?.status);
          }

          // If still zero and we have a phone number, query by phone
          if (!total) {
            const phoneToUse = (user?.phone) || (JSON.parse(localStorage.getItem('user'))?.phone) || (sellerProfile?.phone);
            if (phoneToUse) {
              try {
                const phoneRes = await api.get(`/products/count/by-phone/${encodeURIComponent(phoneToUse)}`);
                const phoneTotal = phoneRes?.data?.totalProducts || 0;
                if (phoneTotal) total = phoneTotal;
              } catch (phoneErr) {
                console.warn('Phone-based count failed:', phoneErr?.response?.status);
              }
            }
          }

          if (!total) {
            const allRes = await api.get('/products?limit=1');
            const globalTotal = allRes?.data?.pagination?.totalProducts || 0;
            total = globalTotal;
          }
  ...
  const handleListProduct = () => {
    // Navigate to pg2 product creation page
