...
// Count products by seller phone number
exports.countByPhone = async (req, res) => {
  try {
    ...
    if (!user) {
      return res.json({ success: true, phone, totalProducts: 0 });
    }

    const total = await Product.countDocuments({ seller: user._id });
    return res.json({ success: true, phone, userId: String(user._id), totalProducts: total });
  } catch (error) {
    res.status(500).json({ 
      success: false, 
      message: 'Server error', 
